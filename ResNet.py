...
class Batch():
    def __init__(self):
        ...
        print "testx:", np.shape(self.train_x),np.shape(self.test_x)
    ...
    def load_table(self, file_path):
        ...
        for row in csv_reader:
            ...
            if np.array_equal(np.array([33,280]),shape1):
                ...
                '''
                plt.plot(np.asarray(map(float, row[2:])))
                plt.show()
                plot += 1'''
...
def inference(input_tensor_batch, n,reuse,is_trainning = True):
    ...
    '''for i in range(n):
        with tf.variable_scope('conv6_%d' % i, reuse=reuse):
            conv6 = residual_block(layers[-1], 512,is_training = is_trainning)
            layers.append(conv6)
        assert conv6.get_shape().as_list()[1:] == [8, 8, 512]'''
    ...
    def train(self):
        ...
        # Want to validate once before training. You may check the theoretical validation
            ...
            if step % FLAGS.report_freq == 0:
                ...
                format_str = ('step %d, magn is %d,loss = %.4f (%.1f examples/sec; %.3f ' 'sec/batch)')
                print(format_str % (step, FLAGS.magn,train_loss_value, examples_per_sec,
                                    sec_per_batch))
            ...
            print('Final validation error = ', np.mean(val_error_list[-5:]))
            print('----------------------------')
